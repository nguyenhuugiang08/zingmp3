            ...
            {podcast.items.map(item => (
              <SwiperSlide key={item.encodeId} >
                <div className='podcastH-wrapper' >
                  <div className='podcastH-wrapper-background' style={{ backgroundImage: `url(${item.thumbnail})` }}></div>
                  <div className='podcastH-wrapper-overlay'></div>
                  <div className='podcastH-wrapper-left'>
                    <div className='podcastH-wrapper-left__img' style={{ backgroundImage: `url(${item.thumbnail})` }}></div>
                  </div>
                  <div className='podcastH-wrapper-right'>
                    <div className='podcastH-wrapper-right__name'>{item.artists[0].name}</div>
                    <div className='podcastH-wrapper-right__title'>{item.title}</div>
                    <div className='podcastH-wrapper-right__description'>{item.description}</div>
                  </div>
                </div>
              </SwiperSlide>
            ))}
