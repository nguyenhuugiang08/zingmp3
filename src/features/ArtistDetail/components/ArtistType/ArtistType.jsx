...
function ArtistType() {
    ...
    useEffect(() => {
        if (data.sections && data.sections.length > 0) {
            setLike(data.sections.filter(item => item.sectionType === 'artist')[0].items)
        }
    }, [data])
    ...
    return (
        <div className={style.top100Outstanding}>
            <div className={style.mainTitle}>
                <div className={style.top100OutstandingTiltle}>{data.sections && data.sections.filter(item => item.sectionType === 'artist')[0].title}</div>
                <Link to="/Top100">
