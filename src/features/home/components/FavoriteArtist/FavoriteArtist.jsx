...
function FavoriteArtist({ data }) {
    ...
    useEffect(() => {
        if (data.length > 0) {
            setFavoriteArtist(data.filter(item => item.sectionType === 'mix')[0].items)
        }
    }, [data])
    ...
    return (
        <div className='favorite-artist'>
            <div className='favorite-artist__title'>{data.length > 0 && data.filter(item => item.sectionType === 'mix')[0].title}</div>
